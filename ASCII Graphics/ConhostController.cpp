...
namespace hcon {
	...
	ConhostControllerImpl::ConhostControllerImpl() {
	...
	void ConhostControllerImpl::switch_target_buffer() {
		target_buffer += 1;
		if (target_buffer >= buffers.size()) target_buffer = 0;
	}

	void ConhostControllerImpl::switch_visible_buffer() {
		visible_buffer += 1;
		if (visible_buffer >= buffers.size()) visible_buffer = 0;
		SetConsoleActiveScreenBuffer(buffers[visible_buffer].handle());
	}

	void ConhostControllerImpl::write(std::string s) {
		buffers[target_buffer].write(s);
	}

	void ConhostControllerImpl::write(char ch) {
		buffers[target_buffer].write(std::to_string(ch));
	}

	void ConhostControllerImpl::set_fcolor(Color c) {
		buffers[target_buffer].set_fcolor(c);
	}

	void ConhostControllerImpl::set_fcolor(int r, int g, int b) {
		set_fcolor(Color{ r,g,b });
	}

	void ConhostControllerImpl::set_bcolor(Color c) {
		buffers[target_buffer].set_bcolor(c);
	}

	void ConhostControllerImpl::set_bcolor(int r, int g, int b) {
		set_bcolor(Color{ r,g,b });
	}

	void ConhostControllerImpl::reset_colors() {
		buffers[target_buffer].reset_colors();
	}

	Coord ConhostControllerImpl::cursor_position() {
		return to_coord(buffers[target_buffer].cursor_pos());
	}

	void ConhostControllerImpl::move_cursor_to(short x, short y) {
		buffers[target_buffer].set_cursor_pos(x, y);
	}

	void ConhostControllerImpl::move_cursor_to(Coord new_pos) {
		move_cursor_to(new_pos.x, new_pos.y);
	}

	void ConhostControllerImpl::maximize() {
		ShowWindow(window, SW_MAXIMIZE);
	}

	Coord ConhostControllerImpl::canvas_size() {
		return to_coord(buffers[target_buffer].buffer_size());
	}

	short ConhostControllerImpl::canvas_width() {
		return canvas_size().x;
	}

	short ConhostControllerImpl::canvas_height() {
		return canvas_size().y;
	}

	Coord ConhostControllerImpl::font_size() {
		return to_coord(buffers[target_buffer].font_size());
	}

	void ConhostControllerImpl::set_resolution(short width, short height) {
	...
	void ConhostControllerImpl::set_buffer_size(short width, short height) {
		buffers[target_buffer].set_buffer_size(width, height);
	}

	void ConhostControllerImpl::set_bufferwindow_size(short width, short height) {
		buffers[target_buffer].set_bufferwindow_size(width, height);
	}

	void ConhostControllerImpl::set_font_size(short width, short height) {
	...
	ConhostController::ConhostController() {
		pimpl = new ConhostControllerImpl{};
	}
	ConhostController::~ConhostController() {
		delete pimpl;
	}
